...
#ifndef _HTL_RB_TREE_H
#define _HTL_RB_TREE_H
...
struct __rb_tree_base_iterator
{
	...
	void increment() {
		if (node->right != 0) {
			...
			if (node->right != y)		// header traits: when node == header
				node = y;
		}
	}

	void decrement() {
		if (node->color == RED && node->parent->parent == node)// header traits: when node == header
			node = node->right;
		else if (node->left != 0) {
...
inline bidirection_iterator_tag
...
__rb_tree_rebalance_for_erase(__rb_tree_node_base* z, __rb_tree_node_base*& root,
