...
#ifndef _HTL_HASHTABLE_H
#define _HTL_HASHTABLE_H
...
template<class Value, class Key, class HashFcn,
	class ExtractKey, class EqualKey, class Alloc = alloc> class hashtable;

template<class Value, class Key, class HashFcn,
	class ExtractKey, class EqualKey, class Alloc> struct __hashtable_iterator;

template<class Value, class Key, class HashFcn,
	class ExtractKey, class EqualKey, class Alloc> struct __hashtable_const_iterator;
...
// 
static const int __htl_num_primes = 28;
static const unsigned long __htl_prime_list[__htl_num_primes] = {
	53,         97,           193,         389,       769,
...
//从上表中获得不小于n的最小质数
inline unsigned long __htl_next_prime(unsigned long n) {
	const unsigned long* first = __htl_prime_list;
	const unsigned long* last = __htl_prime_list + __htl_num_primes;
...
public:
	...
	size_type max_bucket_count()const {
		return __htl_prime_list[__htl_num_primes - 1];
	}
...
private:
	size_type next_size(size_type n)const { return __htl_next_prime(n); }
	...
	// 这组函数用于根据key值或value值获取对应的buckets中的槽位
	...
	void erase_bucket(const size_type n, node* first);

	void copy_from(const hashtable& ht);

};

// hashtableleft == htl == htr == hashtableright
template<class V, class K, class HF, class Ex, class Eq, class A>
bool operator==(const hashtable<V, K, HF, Ex, Eq, A>& htl, const hashtable<V, K, HF, Ex, Eq, A>& htr) {
	if (htl.buckets.size() != htr.buckets.size())
		return false;

	typedef typename hashtable<V, K, HF, Ex, Eq, A>::node node;

	for (int n = 0; n < htl.buckets.size(); ++n) {
		node* cur1 = htl.buckets[n];
		node* cur2 = htr.buckets[n];
...
template <class V, class K, class HF, class Ex, class Eq, class A>
std::pair<typename hashtable<V, K, HF, Ex, Eq, A>::iterator,
	typename hashtable<V, K, HF, Ex, Eq, A>::iterator>
...
template <class V, class K, class HF, class Ex, class Eq, class A>
std::pair<typename hashtable<V, K, HF, Ex, Eq, A>::const_iterator,
